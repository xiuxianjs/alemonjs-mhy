...
const RETCODE_MESSAGES: Record<number, string> = {
  ...
  [1034]: '遇到验证码，请稍后再试或前往米游社App完成验证',
  [10035]: '遇到验证码，请稍后再试或前往米游社App完成验证',
  [5003]: '接口数据异常，请稍后重试',
...
const formatRetcode = (res: MysApiResponse): string => {
  const known = RETCODE_MESSAGES[res.retcode];

  if (known) {
    return `${known} (retcode:${res.retcode})`;
  }

  return `接口返回错误 [${res.retcode}] ${res.message}`;
};
