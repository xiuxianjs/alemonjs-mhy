import { UI_ICONS, BG_IMAGES } from '../../assets/img/index.js';
...
function Row({ icon, label, value, sub, isFull, isLast }) {
    ...
    return (React.createElement("div", { style: isLast ? styles.rowLast : styles.row },
        React.createElement("div", { style: styles.label },
            isUrl ? React.createElement("img", { src: icon, style: { width: '20px', height: '20px' } }) : React.createElement("span", null, icon),
...
function GsCard({ data }) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Row, { icon: UI_ICONS.resin, label: '\u539F\u7CB9\u6811\u8102', value: `${data.current_resin} / ${data.max_resin}`, sub: resinFull ? '已满' : `${formatTime(resinSec)} · ${formatRecoverAt(resinSec)}`, isFull: resinFull }),
        ...
        React.createElement(Row, { icon: UI_ICONS.checkin, label: '\u6BCF\u65E5\u59D4\u6258', value: `${data.finished_task_num} / ${data.total_task_num}`, isFull: data.finished_task_num >= data.total_task_num }),
        React.createElement(Row, { icon: UI_ICONS.primogem, label: '\u6D1E\u5929\u5B9D\u94B1', value: `${data.current_home_coin} / ${data.max_home_coin}`, sub: coinFull ? '已满' : formatTime(coinSec), isFull: coinFull }),
